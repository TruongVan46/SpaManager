...
class ReportService:
    ...
    def get_revenue_report(start_date, end_date):
        ...
        revenue = WorkspaceService.scoped_query(Invoice).with_entities(db.func.sum(Invoice.total_amount)).filter(
    ...
    def get_appointments_by_status(start_date, end_date):
        ...
        start_date = _coerce_report_date(start_date)
        end_date = _coerce_report_date(end_date)
        appointments = WorkspaceService.scoped_query(Appointment).filter(
