...
def _apply_table_widths(ws, skip_rows=(1,)):
...
def _format_date_range(from_date=None, to_date=None):
...
def generate_invoice_excel(invoices, summary):
    ...
    _apply_table_widths(ws, skip_rows=(1,))
...
def generate_statistics_excel(summary, customer_stats, service_stats, from_date=None, to_date=None):
    wb = Workbook()
    ...
    font_bold = Font(name=font_family, size=11, bold=True)
    font_title = Font(name=font_family, size=16, bold=True)
    font_meta = Font(name=font_family, size=10, italic=True)

    fill_header = PatternFill(start_color="DDEBF7", end_color="DDEBF7", fill_type="solid")
    fill_meta = PatternFill(start_color="F7F7F7", end_color="F7F7F7", fill_type="solid")
    ...
    range_label = _format_date_range(from_date, to_date)
    export_time = local_now().strftime('%d/%m/%Y %H:%M:%S')
    ...
    ws1['A1'] = "BÁO CÁO THỐNG KÊ SPA"
    ws1['A1'].font = font_title
    ws1.row_dimensions[1].height = 30
    ws1.merge_cells('A1:B1')

    ws1['A2'] = f"Khoảng thời gian: {range_label}"
    ws1['A2'].font = font_meta
    ws1['A2'].fill = fill_meta
    ws1.merge_cells('A2:B2')

    ws1['A3'] = f"Ngày xuất báo cáo: {export_time}"
    ws1['A3'].font = font_meta
    ws1['A3'].fill = fill_meta
    ...
    for row_idx, (indicator, value) in enumerate(data1, start=6):
    ...
    ws2['A2'] = f"Khoảng thời gian: {range_label}"
    ws2['A2'].font = font_meta
    ws2['A2'].fill = fill_meta
    ws2.merge_cells('A2:E2')

    ws2['A3'] = f"Ngày xuất báo cáo: {export_time}"
    ws2['A3'].font = font_meta
    ws2['A3'].fill = fill_meta
    ...
    ws3['A2'] = f"Khoảng thời gian: {range_label}"
    ws3['A2'].font = font_meta
    ws3['A2'].fill = fill_meta
    ws3.merge_cells('A2:D2')

    ws3['A3'] = f"Ngày xuất báo cáo: {export_time}"
    ws3['A3'].font = font_meta
    ws3['A3'].fill = fill_meta
    ...
    _apply_table_widths(ws1, skip_rows=(1, 2, 3))
    _apply_table_widths(ws2, skip_rows=(1, 2, 3))
    _apply_table_widths(ws3, skip_rows=(1, 2, 3))
