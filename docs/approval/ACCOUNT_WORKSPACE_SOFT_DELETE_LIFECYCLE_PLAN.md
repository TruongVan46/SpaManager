...
* **Task 6.5.21 (DONE):**
  * Đã triển khai khôi phục OWNER và toàn bộ workspace đã xóa mềm mà OWNER sở hữu qua `WorkspaceMember.role = "owner"`.
  * Luồng restore giữ nguyên `approval_status`, chỉ bật `is_active` khi trạng thái duyệt là `active`, không thay đổi `WorkspaceMember.status` và không tác động dữ liệu nghiệp vụ.
  * OWNER không có workspace deleted liên quan vẫn được khôi phục tài khoản, không tự tạo workspace mới.
  * Chức năng **Xóa vĩnh viễn (Purge)** vẫn chưa được triển khai (nút thao tác vẫn bị vô hiệu hóa trên giao diện).
* **Task 6.5.22 (PASS WITH LIMITATIONS):**
  * Workspace còn co-owner hợp lệ được giữ active khi một OWNER bị xóa mềm; membership và business data không bị cascade mutation.
  * Restore OWNER chỉ khôi phục workspace có deletion provenance khớp `deleted_at` và `deleted_by_id` của cùng sự kiện lifecycle.
  * Automated smoke xác nhận transaction rollback, workspace guards, stale session, cross-workspace isolation và giữ nguyên ID dữ liệu.
  * Purge và `ActivityLog.workspace_id` vẫn ngoài phạm vi; chưa chạy manual production-like PostgreSQL smoke trong task này.
  * Evidence: [OWNER/Workspace Lifecycle Smoke + Security Audit](OWNER_WORKSPACE_LIFECYCLE_SMOKE_SECURITY_AUDIT.md).
* **Task 6.5.23a / 6.5.23b:**
  * Audit xác nhận business permanent-delete từng tồn tại thật cho Customer, Service, Appointment và Invoice nhưng không bắt buộc record phải được soft-delete trước.
  * Legacy route, UI dispatch và registry callback đã bị vô hiệu hóa; các public service methods và auto-cleanup placeholder hiện fail-closed trước mọi mutation.
  * Account/Workspace purge vẫn chưa được triển khai và tiếp tục bị vô hiệu hóa.
  * Evidence: [Business Permanent Delete Disablement](BUSINESS_PERMANENT_DELETE_DISABLEMENT.md).
